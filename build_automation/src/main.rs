...
fn build_pkg() {
    let output = Command::new("wasm-pack")
        ...
        .args(["build", "--target", "web", "--features", "ts-interop"])
        .output()
...
fn generate_ts_types() {
    let output = Command::new("cargo")
        .current_dir("./block-game-clone-backend")
        .args(["test", "-q", "--features", "ts-interop"])
        .output()
...
fn add_types_to_package_json() {
    ...
    fs::rename("./block-game-clone-backend/pkg", "./build_automation/pkg").expect("failed to move");
}
